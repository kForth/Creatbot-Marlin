...
public:
	...
	void openFile(char *name, bool read = true, bool replace_current = true);
  	...
  	void removeFile(char * const name);
	void closefile();
	int16_t get();
	void setIndex(long index);
	
	bool UDiskPrintState, UDiskPauseState, UDiskConn, UDiskOK;
	bool isDir, isFileOpen, recoveryFileisOpen;
	bool saving, logging, aborting;
	char filename[FILENAME_LENGTH], longFilename[LONG_FILENAME_LENGTH];

	bool eof() { return filePos >= fileSize; }
	float percentDoneF() { return (isFileOpen && fileSize) ? (float)filePos / fileSize * 100 : 0; }
	uint8_t percentDone() { return (isFileOpen && fileSize) ? filePos / fileSize * 100 : 0; }
	uint32_t getIndex() { return filePos; }
