...
/* A command operation order includes:
A command code (for the serial port method, two synchronous codes are required before the command code),
...
/* ********************************************************************************************************************* */
/* The auxiliary command (manual 2), not very commonly used or to be compatible with CH375 and CH372, the following      */
/* commands always generate interrupt notifications at the end of the operation, and there is always no output data      */
...
/* ********************************************************************************************************************* */
/* Candidate mode, guidance synchronization code before operation command*/
...
/*   Read-Only   hidden   system   volume_id   Directory   archive   undefined   undefined */
/* File attribute uint8_t (untranslated):  */
/* bit0 bit1 bit2 bit3 bit4 bit5 bit6 bit7 */
/*  只   隐   系   卷   目   存   未定义    */
/*  读   藏   统   标   录   档            */
...
#define MAX_FILE_NAME_LEN (13 + 1) /*The maximum length of the file name, the maximum length is 1 root directory symbol+8 main file name+1 decimal point+3 type name+ending character = 14*/
	...
	typedef union _CH376_CMD_DATA
	{
		...
		struct
		{
			uint32_t mTotalSector; /*Return: The total number of current logic disks (total length of 32 bits, low bytes before)*/
...
/* USB's package label pid, the host method may use */
